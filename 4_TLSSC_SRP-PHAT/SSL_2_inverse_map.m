...
diary('off');
inverse_map_filename= sprintf('./log/inverse_map_p_%02d.txt',pp);
delete(inverse_map_filename);
diary(inverse_map_filename);
fprintf('SRP-PHAT (Inverse mapping) (p=%d)\n',pp);
time= clock;
fprintf('%s, %d:%d\n',date,time(4),time(5));
...
for i=1:nfile
    ...
    for f=1:nFrame
        ...
        I= srp_phat_inverse_map(input_frames, ...
                                inverse_map, ...
